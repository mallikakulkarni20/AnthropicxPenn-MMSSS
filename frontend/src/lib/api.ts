...
export const api = {
  ...
  // Publish new lecture version with all approved changes
  async publishLectureVersion(
    teacherId: string,
    lectureId: string
  ): Promise<{
    newLecture: Lecture;
    ...
    const response = await fetch(
      `${API_BASE_URL}/teacher/${teacherId}/lectures/${lectureId}/publish`,
      {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
    ...
    if (!response.ok) throw new Error("Failed to publish lecture version");
    return response.json();
  },
};
